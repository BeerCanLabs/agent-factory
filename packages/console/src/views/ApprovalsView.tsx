  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-amber-950/60 text-amber-400 border border-amber-800">
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-xl border transition cursor-pointer ${
                    isSelected
                      ? 'bg-slate-850 border-amber-500 shadow-amber-900/20 shadow-lg'
                      : 'bg-slate-900 border-slate-800 hover:border-slate-700'
                  }`}
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-mono text-[10px] text-slate-500">{appr.id}</span>
                    {getRiskBadge(appr.risk)}
                  </div>
                  <h4 className="text-xs font-bold text-white mb-1 truncate">{appr.tool}</h4>
                  <div className="flex items-center justify-between text-[11px] text-slate-400">
                    <span>Agent: <strong className="text-slate-200">{appr.agentId}</strong></span>
            ...
            <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-5">
              <div className="flex items-start justify-between border-b border-slate-800 pb-4">
                <div>
                  <div className="flex items-center space-x-2">
                    <span className="font-mono text-xs text-amber-400 font-bold">{selectedApproval.id}</span>
                    {getRiskBadge(selectedApproval.risk)}
                  </div>
                  <h3 className="text-base font-bold text-white mt-1">{selectedApproval.tool}</h3>
                  <p className="text-xs text-slate-400 mt-0.5">
                    Target Host: <code className="text-slate-300 font-mono">{selectedApproval.host || 'External API'}</code>
                  </p>
                </div>
                <div className="text-right text-xs">
                  <div className="text-slate-500">Originating Run</div>
                  <div className="font-mono text-slate-200 font-semibold">{selectedApproval.runId}</div>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-xs font-semibold text-slate-300">
                  ...
                  <span className="text-[10px] text-slate-500">Read-Only Audit Snapshot</span>
                </div>
                <pre className="bg-slate-950 border border-slate-800 rounded-lg p-3 text-xs font-mono text-emerald-300 overflow-x-auto max-h-56">
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="flex items-center justify-end space-x-3 pt-2">
                  <button
                    ...
                    className="px-4 py-2 bg-red-900/60 hover:bg-red-800 disabled:opacity-40 text-red-200 border border-red-700/80 rounded-lg text-xs font-semibold flex items-center space-x-1.5 transition"
                  >
                  ...
                  <button
                    ...
                    disabled={isProcessing || !permissions.canApproveTool}
                    className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-white rounded-lg text-xs font-semibold flex items-center space-x-1.5 shadow-lg shadow-emerald-950/50 transition"
                  >
