    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <select
                ...
                className="bg-slate-950 text-white font-bold text-base rounded px-2 py-1 border border-slate-800 cursor-pointer focus:outline-none focus:border-emerald-500"
              >
                {agents.map((a) => (
            ...
            <p className="text-xs text-slate-400 mt-0.5">{agent.role || 'Autonomous Cartridge'}</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-4 text-xs">
          <div className="text-right">
            <div className="text-[10px] text-slate-500 uppercase font-semibold">Current State</div>
            <div className="font-bold text-emerald-400">{agent.state}</div>
          </div>
          <div className="h-6 w-px bg-slate-800" />
          <div className="text-right">
            <div className="text-[10px] text-slate-500 uppercase font-semibold">24h Spend</div>
            <div className="font-bold text-white">${(agent.currentSpendUsd || 0).toFixed(2)}</div>
          </div>
          <div className="h-6 w-px bg-slate-800" />
          <div className="text-right">
            <div className="text-[10px] text-slate-500 uppercase font-semibold">Spend Cap</div>
            <div className="font-bold text-slate-300">${agent.spendLimitUsd || 0}/day</div>
          </div>
        </div>
      </div>

      {/* Tabs Header */}
      <div className="border-b border-slate-800 flex space-x-2 text-xs font-semibold">
        {[
      ...
      {activeTab === 'runtime' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4">
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between py-1.5 border-b border-slate-800/80">
                <span className="text-slate-400">Compute Service:</span>
                <span className="font-mono text-slate-200">AWS ECS Fargate</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-slate-800/80">
                <span className="text-slate-400">Network Subnet:</span>
                <span className="font-mono text-slate-200">vpc-factory-private (No IGW / NAT)</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-slate-800/80">
                <span className="text-slate-400">Public IP:</span>
                <span className="font-mono text-emerald-400 font-semibold">None (0.0.0.0/0 blocked)</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-slate-800/80">
                <span className="text-slate-400">Egress Conduit:</span>
                <span className="font-mono text-slate-200">http://factory-gateway:3001</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-slate-800/80">
                <span className="text-slate-400">Warm-Down Timer:</span>
                <span className="font-mono text-slate-200">{agent.warmDownSeconds || 300} seconds</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4">
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between py-1.5 border-b border-slate-800/80">
                <span className="text-slate-400">Doorman Gateway:</span>
                <span className="text-emerald-400 font-semibold">Online (24/7 WebSocket)</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-slate-800/80">
                <span className="text-slate-400">Ingress Channels:</span>
                <span className="font-mono text-slate-200">Discord Gateway & API Mailbox</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-slate-800/80">
                <span className="text-slate-400">Scale-to-Zero Strategy:</span>
                <span className="text-slate-300">Awakened on mention / sleep on idle</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-slate-800/80">
                <span className="text-slate-400">Last Task Wakeup:</span>
                <span className="text-slate-300">{agent.lastStateChange || 'Recently'}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'terminal' && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-4">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="text-slate-500 font-mono text-[10px]">Pipes: stdout/stderr & mailbox</span>
          </div>

          <div className="bg-slate-950 border border-slate-800 rounded-lg p-3 font-mono text-xs text-slate-300 h-64 overflow-y-auto space-y-1">
            {logs.map((line, idx) => (
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-white rounded-lg text-xs font-semibold flex items-center space-x-1.5 transition"
            >
      ...
      {activeTab === 'memory' && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4">
          <div className="flex items-center space-x-2">
            ...
            <h4 className="text-sm font-bold text-white">The "Notebook & Safe" Architecture</h4>
          </div>
          <p className="text-xs text-slate-400 leading-relaxed">
            Cartridges write strictly to an embedded SQLite database in <code className="text-emerald-400">$MEMORY_DIR</code>.
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
            <div className="bg-slate-950 border border-slate-800 rounded-lg p-3 text-xs">
              ...
              <span className="text-[10px] text-emerald-400 font-semibold">Sub-second S3 sync guaranteed</span>
            </div>

            <div className="bg-slate-950 border border-slate-800 rounded-lg p-3 text-xs">
              <span className="text-slate-500">WAL Mode & Timeout</span>
              <div className="text-lg font-bold text-emerald-400 mt-1">PRAGMA WAL</div>
              <span className="text-[10px] text-slate-400">busy_timeout=5000ms</span>
            </div>

            <div className="bg-slate-950 border border-slate-800 rounded-lg p-3 text-xs">
              <span className="text-slate-500">Atomic Checkpoint</span>
              <div className="text-lg font-bold text-white mt-1">TRUNCATE</div>
              <span className="text-[10px] text-slate-400">No dangling -wal lock artifacts</span>
            </div>
          </div>

          <div className="bg-slate-950 border border-slate-800 rounded-lg p-3 text-xs space-y-1">
            <div className="text-slate-400 font-semibold">S3 Remote Mind Vault:</div>
            <div className="font-mono text-slate-300 break-all">{agent.mindPrefix || `s3://beercanlabs-minds/${agent.id}/`}</div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'models' && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
            <div>
              <h4 className="text-sm font-bold text-white">Active Production Model</h4>
              <p className="text-xs text-slate-400">Hot-swap active reasoning engine at the Egress Gateway</p>
            </div>
            ...
            <div className="flex items-center space-x-2">
              <select
                ...
                className="bg-slate-950 text-white font-mono text-xs rounded-lg px-3 py-2 border border-slate-800 focus:outline-none focus:border-emerald-500"
              >
              ...
              {(agent.approvedModels || [agent.model]).map((m) => (
                ...
                >
                  ...
                  <div className="flex items-center space-x-4">
                    <span className="text-slate-400 text-[11px]">Bench Score: <strong className="text-slate-200">100%</strong> (15/15)</span>
                    <span className="text-slate-400 text-[11px]">Avg Cost: <strong className="text-slate-200">$0.021/turn</strong></span>
                  </div>
                </div>
              ))}
            </div>
      ...
      {activeTab === 'lifecycle' && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-6">
          <div>
            <h4 className="text-sm font-bold text-white">Two-Stage Retirement & Purge Lifecycle</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Stage 1: Soft-Retire */}
            <div className="bg-slate-950 border border-amber-800/40 rounded-xl p-4 space-y-3">
            ...
            <div className="bg-slate-950 border border-red-800/40 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between">
