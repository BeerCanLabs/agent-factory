...
export function verifyChain(
  ...
  for (const c of checkpoints) {
    const row = chain[c.toSeq - 1];
    if (!row) return { ok: false, rows: chain.length, firstBadSeq: chain.length + 1, reason: `rows missing: checkpoint covers seq ${c.toSeq}` };
    if (row.hash !== c.hash) {
      const alternatives = checkpoints.filter((other) => other.toSeq === c.toSeq);
      if (alternatives.some((alt) => alt.hash === row.hash)) continue;
      return { ok: false, rows: chain.length, firstBadSeq: c.toSeq, reason: 'chain diverges from WORM checkpoint' };
    }
  }
  return { ok: true, rows: chain.length, head: prev, checkpointsChecked: checkpoints.length };
}
