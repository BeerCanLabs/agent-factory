...
build() { docker build --platform linux/amd64 -q -f "$1" ${3:+--build-arg CARTRIDGE=$3} -t "$REGISTRY/$2:$TAG" "$ROOT" >/dev/null && docker push -q "$REGISTRY/$2:$TAG" >/dev/null && echo "  $2:$TAG"; }
...
api() { curl -sS --fail-with-body -H "Authorization: Bearer $ADMIN" -H 'content-type: application/json' "$@"; }
[ "$(curl -s -o /dev/null -w '%{http_code}' "http://$FACTORY_DOMAIN/healthz")" = 301 ] && echo "  ok  plain HTTP redirects"
curl -sS --fail "$CP/healthz" >/dev/null && echo "  ok  HTTPS health"
[ "$(curl -s -o /dev/null -w '%{http_code}' "$CP/api/v1/agents")" = 401 ] && echo "  ok  unauthenticated 401"
